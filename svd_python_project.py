...
def forgo():
    f = open("svdp.csv")  # to open file

    data = csv.reader(f)  # to read the file in data variable
    cp = 1  # all other steps are already explained in code below because this is used
    # many times
    cu = 1
    ...
    for r in data:
        ...
        if (r[0] == p1.get()):
            ...
            if (r[2] == p2.get()):
                cp = 0
                Label(screen12, text="your password is " + r[1], font=("bold", 20), bg="grey").pack()

                break
    f.close()
    if (cu == 1):
        Label(screen12, text="wrong ussername", font=("bold", 20), bg="grey").pack()
    elif (cp == 1):
        Label(screen12, text="incorrect email", font=("bold", 20), bg="grey").pack()


def forgot():
    global screen12  # global is used so that we can call this screen from other screens

    screen12 = Toplevel(screen)
    screen12.title("menu")  # title
    screen12.configure(background="grey")
    global p1
    global p2
    p1 = StringVar()
    p2 = StringVar()
    Label(screen12, text="username", font=("bold", 20), bg="grey").pack()
    Entry(screen12, bd=10, textvariable=p1, bg="pink", width=30).pack()
    Label(screen12, text="email", font=("bold", 20), bg="grey").pack()
    Entry(screen12, bd=10, textvariable=p2, bg="pink", width=30).pack()

    Button(screen12, text="check", command=forgo, bd=30, bg="cyan", font=("bold", 15)).pack()
...
def che():
    f = open("svdp.csv")  # this is used to open the file and we have save it on f variable
...
def log():
    ...
    Button(screen3, text="forgot password", command=forgot, bg="cyan", font=("bold", 15), bd=5).pack()
...
def bookin():
    if (vnum.get() == ""):  # if usser will not enter the vehicle number then following label will work
        ...
        if (vehicle.get() == "Two-wheeler"):  # this is used to check which vehicle we have choose
            ...
            Label(screen5, text="Select the type of parking for your Two-wheeler", font=("italic", 20),
                  bg="grey").pack()
            Label(screen5, text="Vehicle number :- " + vnum.get(), font=("italic", 20), bg="grey").pack()
        ...
        global days
        days = StringVar()
        Label(screen5, text="Number of days you want to park your " + vehicle.get(), font=("italic", 20),
              bg="grey").pack()
...
def price():
    if (vehicle.get() == "car" and type.get() == "Normal"):
        ...
        # place is used to give the value of x any u axis where we want to place ou widget
...
def login():
    if (len(e2.get()) < 6):  # if password will be less then 6 charcterthen it will work

        Label(screen1, text="Your password should have atleast 6 characters", font=("bold", 20), fg="red",
              bg="grey").pack()
    else:  # else data will be saved in our file named svd(shubhi,vaibhav,devansh project :\ )
        f = open("svdp.csv", "a+")  # open file in append mode
        f.write(e1.get() + "," + e2.get() + "," + e3.get())  # ussername , password will be saved
        f.write("\n")  # for new line
        f.close()  # close file
...
def reg():
    ...
    Entry(screen1, bd=5, bg="pink", width=30, textvariable=e3).pack()
...
main_screen()  # all the code is written b|w from tkinter import* and main_screen()